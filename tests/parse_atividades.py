"""
🧪 TESTE: Parse de Exportacao Atividade.xlsx para JSON
...
def main():
    """
    Parse de Exportacao Atividade.xlsx para JSON - SEM inserção em banco
    """
    # Localizar arquivo
    if len(sys.argv) > 1:
        ...
        project_downloads = os.path.join(ROOT, 'downloads', 'Exportacao Atividade.xlsx')
        user_download_dir = os.path.join(os.path.expanduser('~'), 'Downloads')
        user_downloads = os.path.join(user_download_dir, 'Exportacao Atividade.xlsx')
        
        file_path = project_downloads if os.path.exists(project_downloads) else user_downloads

    if not os.path.exists(file_path):
        print(f"❌ Arquivo não encontrado: {file_path}")
        print(f"Uso: python {sys.argv[0]} [caminho_para_ExportacaoAtividade.xlsx]")
        sys.exit(2)
    ...
    print(f"✅ Parsed {len(records)} records de ExportacaoAtividade.xlsx")
    ...
    out_file = os.path.join(out_dir, f'parsed_atividades_{ts}.json')
