"""
Teste de envio de ATIVIDADES para SQL Server

Processa um arquivo Excel de Atividades (ExportacaoAtividade.xlsx) ou JSON,
parseia os dados e envia para a tabela EXPORTACAO_ATIVIDADE no SQL Server.
...
Uso:
    python tests/test_post_sql_atividades.py                  # Usa arquivo Excel recente em downloads/
    python tests/test_post_sql_atividades.py --file ./meu.xlsx # Arquivo Excel específico
    python tests/test_post_sql_atividades.py --json file.json  # Arquivo JSON parseado
    python tests/test_post_sql_atividades.py --dry-run         # Simula envio
    python tests/test_post_sql_atividades.py --batch-size 10   # Batch customizado
"""
...
parser = argparse.ArgumentParser(
    description='Teste de envio de ATIVIDADES para SQL Server',
    formatter_class=argparse.RawDescriptionHelpFormatter,
    epilog="""
Exemplos:
    # Usa arquivo Excel recente em downloads/
    python test_post_sql_atividades.py
    
    # Arquivo Excel específico
    python test_post_sql_atividades.py --file ../../downloads/ExportacaoAtividade.xlsx
    
    # Simula envio (DRY_RUN)
    python test_post_sql_atividades.py --dry-run
    
    # Batch customizado
    python test_post_sql_atividades.py --batch-size 50
    
    # Arquivo JSON parseado
    python test_post_sql_atividades.py --json ./parsed_atividades.json
    """
)

parser.add_argument(
    '--file', '-f',
    help='Arquivo Excel (default: ExportacaoAtividade.xlsx em downloads/)',
    default=None
)
parser.add_argument(
    '--json',
...
# Importar funções
from app import parse_export_atividades, post_records_to_mssql, logger

print("\n" + "="*70)
print("🧪 TESTE: Envio de ATIVIDADES para SQL Server")
print("="*70)
...
if args.json:
    ...
    if args.file:
        ...
        files = list(downloads_dir.glob('Exportacao*Atividade*.xlsx'))
        if not files:
            print(f"\n❌ Nenhum arquivo 'Exportacao*Atividade*.xlsx' encontrado em {downloads_dir}")
    ...
    try:
        records = parse_export_atividades(str(excel_file))
...
print(f"   Tabela: EXPORTACAO_ATIVIDADE")
...
try:
    ...
    result = post_records_to_mssql(
        ...
        file_name='ExportacaoAtividade.xlsx'
    )
    ...
    print(f"\n📋 LOGS GERADOS:")
    print(f"   • logs/sent_records_atividades.jsonl (registros enviados)")
    ...
    print(f"   • Tabela EXPORTACAO_ATIVIDADE existe no banco")
    sys.exit(1)
