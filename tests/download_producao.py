"""
Teste de download individual - Relatório de Produção

Uso:
    python tests/test_download_producao.py [--headless] [--timeout 60]
...
def test_download_producao(headless=False, timeout=60):
    """
    Testa download individual do relatório de Produção.
    ...
    try:
        ...
        logger.info("=" * 70)
        logger.info("🧪 TESTE: Download Individual - Produção")
        logger.info("=" * 70)
        ...
        # Download
        logger.info("[5/5] Iniciando download de Produção...")
        exportProducao(driver)
        time.sleep(2)
        
        logger.info("=" * 70)
        logger.info("✅ SUCESSO: Download de Produção concluído!")
        logger.info("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download individual: Relatório de Produção",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplos de uso:
  python tests/test_download_producao.py              # Download com UI visível
  python tests/test_download_producao.py --headless   # Modo headless (sem UI)
  python tests/test_download_producao.py --timeout 120 # Timeout de 120s
        """
    )
    ...
    # Executar teste
    success = test_download_producao(
